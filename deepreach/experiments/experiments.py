...
class Experiment(ABC):
    ...
    def validate(self, device, epoch, save_path, x_resolution, y_resolution, z_resolution, time_resolution):
        ...
        # handle 2D case (no z-slice) if z_axis_idx <0
        ...
        for i in range(len(times)):
            for j in range(len(zs)):
                ...
                if z_idx >= 0:
                    ...
                    ax = fig.add_subplot(len(zs), len(times), 1 + i)
                    ax.set_title('t = %0.2f' % (times[i]))

                s = ax.imshow(1*(values.detach().cpu().numpy().reshape(x_resolution, y_resolution).T <= 0), cmap='bwr', origin='lower', \
                              extent=(x_min, x_max, y_min, y_max))      # it was (-1., 1., -1., 1.) before
                fig.colorbar(s) 
